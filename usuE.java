...
public class usu {
      ...
      while (!(fromUser =  br.readLine()).equals("q")) {
        if (fromUser != null) {
          if (verificarComando(fromUser)) {
            alias = getResponse(c, fromUser, alias, gpt, br);
      ...
      while ((fromFile =  br.readLine()) != null) {
	    ...
	    if (autenticado) {
	       ...
	       gpt.interrupt();
            ...
            alias = getResponse(c, fromFile, alias, gpt, br);
          }
        } else {
  ...
  public static String getResponse(Msg c, String clientRequest, String aliasUsu, ThreadGroup gpt, BufferedReader in) { 
    try {
      String mensaje = "";
